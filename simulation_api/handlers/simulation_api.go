...
var cancellableTaskStatuses = map[string]struct{}{
	"queued":      {},
	"dispatching": {},
	"running":     {},
}
...
func ListProjects(c *gin.Context) {
	...
	for _, project := range projects {
		...
		if derivedStatus, err := deriveProjectStatus(project.ID); err == nil && derivedStatus != "" {
...
func ListTasks(c *gin.Context) {
	...
	if err := config.DB.
		Joins("Project").
		Where("Project.tenant_id = ?", user.TenantID).
		Order("simulation_tasks.created_at desc").
		Preload("Project").
...
func ListResults(c *gin.Context) {
	...
	items := make([]ResultListItem, 0, len(tasks))
	for _, task := range tasks {
...
func CancelTask(c *gin.Context) {
	...
	if _, allowed := cancellableTaskStatuses[task.Status]; !allowed {
		c.JSON(http.StatusBadRequest, gin.H{"error": "task cannot be cancelled in its current state"})
		return
	}
	...
	if err := config.DB.Model(&models.SimulationTask{}).
		Where("project_id = ? AND status IN ?", task.ProjectID, []string{"queued", "dispatching", "running"}).
		Count(&activeTaskCount).Error; err == nil && activeTaskCount > 0 {
		projectStatus = "running"
	}
	_ = config.DB.Model(&models.SimulationProject{}).
		Where("id = ?", task.ProjectID).
		Updates(map[string]any{"status": projectStatus, "update_at": now}).Error
...
func DeleteProject(c *gin.Context) {
	...
	var activeTaskCount int64
	if err := config.DB.Model(&models.SimulationTask{}).
		Where("project_id = ? AND status IN ?", project.ID, []string{"queued", "dispatching", "running"}).
		Count(&activeTaskCount).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "查询项目任务失败: " + err.Error()})
		return
	}
	if activeTaskCount > 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "project has active tasks and cannot be deleted"})
		return
	}
	...
	if err := config.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("project_id = ?", project.ID).Delete(&models.SimulationConfig{}).Error; err != nil {
			return err
		}
		if err := tx.Delete(&models.SimulationProject{}, project.ID).Error; err != nil {
			return err
		}
		if project.MapDataID != 0 {
			if err := tx.Delete(&models.MapData{}, project.MapDataID).Error; err != nil {
				return err
			}
		}
		return nil
